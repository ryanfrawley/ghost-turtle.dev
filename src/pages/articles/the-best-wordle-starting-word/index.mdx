...
Adding up the totals for each word, we get a positive difference of 4.1% by taking letter positions into account!
...
What if we could feed the results of the first guess back into our program? One way to do this would be to use the matching and missed letters to reduce the number of available words, then recalculate the frequencies like we did for the first guess. Each subsequent guess will narrow down the potential words and have a new set of words with the highest letter and positional frequencies.
